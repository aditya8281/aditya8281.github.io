...
export default function ExperienceSection() {
  return (
    <section id="experience" className="border-t border-white/5 bg-slate-950/40 px-6 py-20 sm:px-8">
                  ...
                  {item.highlights?.length ? (
                    <ul className="mt-6 grid gap-3 text-sm text-slate-200 sm:grid-cols-2">
                      {item.highlights.map((highlight) => (
                        <li key={highlight} className="rounded-3xl border border-cyan-400/10 bg-slate-950/70 px-4 py-3">
                          {highlight}
                        </li>
                      ))}
                    </ul>
